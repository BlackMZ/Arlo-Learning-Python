...
class PamConnector:
    ...
    def discover_account(self):
        ...
        try:
            ...
            img_element = self.driver.find_element_by_id("captcha_img")
            if img_element is not None:
                img_txt = self.get_img_txt(img_element.screenshot_as_base64)
                # 填充验证码输入框
                self.driver.find_element_by_name("verify").send_keys(img_txt)
                time.sleep(2)

            check = self.driver.find_element_by_id("privacyEnable").is_selected()
            if check is False:
                self.driver.find_element_by_id("privacyEnable").click()
            ...
            if self.driver.find_element_by_id("gcs_user_area").is_enabled():
                ...
                time.sleep(3)
                self.driver.find_element_by_xpath("//a[contains(.,'管理员帐号')]").click()
                time.sleep(3)
                # //*[@id="ext-gen255"]/div/table/tbody/tr/td[3]/div/div/a
                discover_username_elements = self.driver.find_elements_by_xpath(
                    "//div/table/tbody/tr/td[3]")
