...
class PamConnector:
    ...
    def discover_account(self):
        ...
        try:
            ...
            img_element = self.driver.find_element_by_class_name("uedc-ppkg-login_captcha")
            if img_element is not None:
                img_txt = self.get_img_txt(img_element.screenshot_as_base64)
                # 填充验证码输入框
                self.driver.find_element_by_name("captcha").send_keys(img_txt)
                time.sleep(1)

            check = self.driver.find_element_by_name("checkbox").is_selected()
            if check is False:
                self.driver.find_element_by_name("checkbox").click()
            self.driver.find_element_by_xpath("//button[contains(.,'立即登录')]").click()
            time.sleep(3)
            if self.driver.find_element_by_id("top_nav").is_enabled():
                time.sleep(1)
                self.driver.find_element_by_xpath("//span[contains(.,'系统')]").click()
                time.sleep(1)
                self.driver.find_element_by_xpath("//a[contains(.,'管理员账号')]").click()
                time.sleep(3)
                # //*[@id="sfext3-gen439"]/div[1]/table/tbody/tr/td[3]/div/span
                discover_username_elements = self.driver.find_elements_by_xpath(
                    "//table/tbody/tr/td[3]/div/span")
