...
class PamConnector:
    ...
    def verify_login(self):
        ...
        try:
            ...
            time.sleep(3)
            self.driver.find_element_by_id("pwd_username").send_keys(self.user)
            time.sleep(2)
            self.driver.find_element_by_id("pwd_pwd").send_keys(self.pwd)
            time.sleep(2)
            img_element = self.driver.find_element_by_xpath("//*[@id='sign-box']/form[1]/div[3]/div/div/img")
            if img_element is not None:
                img_txt = self.get_img_txt(img_element.screenshot_as_base64)
                self.driver.find_element_by_id("pwd_captcha").send_keys(img_txt)
                time.sleep(1)

            self.driver.find_element_by_xpath("//button[contains(.,'登 录')]").click()
            time.sleep(2)
            result = self.driver.find_element_by_class_name("right___1LIhw").is_enabled()
            if result:
                print("result=" + "true")
            else:
                print("result=" + "false")
