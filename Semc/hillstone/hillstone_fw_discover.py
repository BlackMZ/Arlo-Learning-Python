    ...
    def discover_account(self):
        ...
        try:
            ...
            self.driver.find_element_by_xpath(
                "//*[@id='mainContent']/table/tbody/tr/td[2]/table/tbody/tr[5]/td[3]/input").send_keys(self.pwd)
            time.sleep(1)
            self.driver.find_element_by_id("login_button").click()
            time.sleep(5)
            if self.driver.find_element_by_class_name("banner_info").is_enabled():
                time.sleep(5)
                self.driver.find_element_by_xpath("//span[contains(.,'系统')]").click()
                time.sleep(15)
                self.driver.find_element_by_xpath("//a[contains(.,'设备管理')]").click()
                time.sleep(5)
                # //*[@id="ext-gen2443"]/div
                ...
                for i in range(0, len(discover_username_elements)):
                    if len(discover_username_elements[i].text) == 0:
                        continue
