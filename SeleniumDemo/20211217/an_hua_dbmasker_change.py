    ...
    def verify_modify_pwd(self):
        ...
        try:
            ...
            if self.driver.find_element_by_id("top-user").is_enabled():
                self.driver.find_element_by_id("top-user").click()
                time.sleep(1)
                self.driver.find_element_by_xpath("//span[contains(.,'修改密码')]").click()
                time.sleep(1)
                # frame
                self.driver.switch_to.frame("changeUserPasswordDialogFrame")
                time.sleep(1)

                cur_pwd = self.driver.find_element_by_id("oldpwd")
                cur_pwd.clear()
                cur_pwd.send_keys(self.pwd)
                time.sleep(1)
                new_pwd1 = self.driver.find_element_by_id("upwd")
                new_pwd1.clear()
                new_pwd1.send_keys(self.new_pwd)
                time.sleep(1)
                new_pwd2 = self.driver.find_element_by_id("confirmPwd")
                new_pwd2.clear()
