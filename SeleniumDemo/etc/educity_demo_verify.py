...
class PamConnector:
    ...
    def verify_login(self):
        ...
        try:
            ...
            self.driver.find_element_by_xpath("//li[text()='密码登录']").click()
            ...
            # 修改验证码xpath
            img_element = self.driver.find_element_by_xpath("//img[@title='点击刷新验证码']")
            if img_element is not None:
                img_txt = self.get_img_txt(img_element.screenshot_as_base64)
                # 填充验证码输入框
                self.driver.find_element_by_id("validateImgCode").send_keys(img_txt)
                time.sleep(1)

            self.driver.find_element_by_class_name("_uname_log_but_").click()
            time.sleep(1)
            result = self.driver.find_element_by_id("newname").is_enabled()
            if result:
                print("result=" + "true")
            else:
                print("result=" + "false")
    ...
    def get_driver(self):
        if self.type == 'local':
            ...
            self.driver = webdriver.Chrome('C:\Program Files\Google\Chrome\Application\95.0.4638.69\chromedriver', options=options)
