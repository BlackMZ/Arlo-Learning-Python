...
class PamConnector:
    ...
    def verify_modify_pwd(self):
        ...
        try:
            ...
            self.driver.find_element(By.NAME, "Name").send_keys(self.user)
            time.sleep(1)
            self.driver.find_element(By.ID, "Password").send_keys(self.pwd)
            time.sleep(1)
            self.driver.find_element(By.ID, "login").click()
            time.sleep(3)
            if self.driver.find_element(By.ID, "mapspf").is_enabled():
                self.driver.find_element(By.LINK_TEXT, "用户管理").click()
                time.sleep(2)
                self.driver.find_element(By.NAME, "manager_set").click()
                time.sleep(3)
                new_pwd1 = self.driver.find_element(By.ID, "password_set")
                time.sleep(1)
                new_pwd1.send_keys(Keys.CONTROL, 'a')
                time.sleep(1)
                new_pwd1.send_keys(self.new_pwd)
                time.sleep(1)
                self.driver.find_element(By.ID, "manager_update").click()
