...
class PamConnector:
    ...
    def verify_modify_pwd(self):
        ...
        try:
            ...
            time.sleep(1)
            self.driver.find_element(By.ID, "login_pwd").send_keys(self.pwd)
            time.sleep(1)
            self.driver.find_element_by_id("login_btn").click()
            time.sleep(10)
            if self.driver.find_element_by_class_name("close-mask").is_enabled():
                self.driver.find_element_by_class_name("close-mask").click()
            time.sleep(2)
            if self.driver.find_element_by_id("subMenu").is_enabled():
                sub_meun = self.driver.find_element_by_id("subMenu")
                ActionChains(self.driver).move_to_element(sub_meun).perform()
                self.driver.find_element_by_link_text("帐号设置").click()
                time.sleep(1)
                self.driver.find_element_by_link_text("修改密码").click()
                time.sleep(1)
                self.driver.switch_to.frame(0)
                cur_pwd = self.driver.find_element_by_id("oldPassword")
                cur_pwd.clear()
                cur_pwd.send_keys(self.pwd)
                time.sleep(1)
                new_pwd1 = self.driver.find_element_by_id("newPassword")
                new_pwd1.clear()
                new_pwd1.send_keys(self.new_pwd)
                time.sleep(1)
                new_pwd2 = self.driver.find_element_by_id("newPassword1")
                new_pwd2.clear()
                new_pwd2.send_keys(self.new_pwd)
                time.sleep(1)
                self.driver.find_element_by_id("modifypwd_btn").click()
                time.sleep(1)
                self.driver.find_element_by_xpath("//button[contains(.,'确定')]")
                print("result=" + "true")
            else:
                print("result=" + "false")
        except Exception:
