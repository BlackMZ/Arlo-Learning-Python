    ...
    def discover_account(self):
        ...
        try:
            ...
            if self.driver.find_element_by_id("logout-link").is_enabled():
                # https://172.31.250.136/gui/#/system/admin/?page=1&items=25
                # self.driver.get(self.location + "/gui/#/system/admin/?page=1&items=25")
                system = self.driver.find_element_by_link_text("System")
                webdriver.ActionChains(self.driver).move_to_element(system).perform()
                time.sleep(1)
                self.driver.find_element_by_link_text("Admin").click()
                # //*[@id="main-page"]/div[5]/div[1]/div/datatable/div/div[3]/div[2]/div/div[1]
                # total_num_text = self.driver.find_element_by_xpath(
                #     "//*[@id='main-page']/div[5]/div[1]/div/datatable/div/div[3]/div[2]/div/div[1]").text
                # total_num = int(total_num_text[6:(len(total_num_text) - 7)])
                # total_page = int((total_num / 25)) + 1
                time.sleep(5)
                discover_username_elements = self.driver.find_elements_by_xpath(
                    "//table/tbody/tr/td[3]")  # //*[@id='main-page']/div[5]/div[1]/div/datatable/div/div[2]
