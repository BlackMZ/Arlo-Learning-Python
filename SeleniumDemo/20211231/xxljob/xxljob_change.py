...
class PamConnector:
    ...
    def verify_modify_pwd(self):
        ...
        try:
            ...
            time.sleep(1)
            if self.driver.find_element(By.LINK_TEXT, "欢迎 demo").is_enabled():
                self.driver.find_element(By.LINK_TEXT, "欢迎 demo").click()
                time.sleep(1)
                self.driver.find_element(By.LINK_TEXT, "修改密码").click()
                time.sleep(1)
                new_pwd1 = self.driver.find_element(By.NAME, "password").send_keys(
                    self.new_pwd)
                time.sleep(1)
                self.driver.find_element(By.CSS_SELECTOR, ".col-sm-offset-3 > .btn-primary").click()
                time.sleep(1)
                print("result=" + "true")
            else:
                print("result=" + "false")
